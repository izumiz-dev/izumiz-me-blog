...
### タイムラインバッジ（種別識別色、`TimelineCard.astro` 内リテラル）

タイムラインの各カード種別を示すバッジのみに使う専用色。他の用途には流用しない。

| 種別    | グラデーション                                     |
| ------- | -------------------------------------------------- |
| Blog    | `#6fb5ff → #2b7fe0 → #1b5fb0`（`--aqua-*` と同値） |
| Gallery | `#c4a5ff → #8b5cf6 → #6d3fd4`                      |
| Books   | `#fbbf24 → #d97706 → #b45309`                      |
...
### タイムラインカード（`TimelineCard.astro`）

- 額装レシピ（クローム縁 + inset ハイライト + 影）を薄めた frosted なカード。
  種別バッジ（右上、角丸 `--radius-tight`）でタイムラインバッジ配色を使い分ける。
  Gallery カードはサムネイル+タイトルのみでモーダルプレビューを開く。
